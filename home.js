                                    ...
                                    if (qs.size === 0) {
                                        ...
                                        db.collection("districts").where("Name","==",district).get().then((qs)=>{
                                            if(qs.size===0){
                                                ...
                                                qs.forEach((doc)=>{
                                                    console.log(doc);
                                                    db.collection("districts").doc(doc.id).update({pincodes:firebase.firestore.FieldValue.arrayUnion(pincode)});
                                                });
                                            }
                                        })
                                    }
    ...
    task.on('state_changed', function(snapshot){
      ...
      console.log(error);
        ...
        task.snapshot.ref.getDownloadURL().then(function(downloadURL) {
          ...
          httpreq('http://localhost:4000/visionai',JSON.stringify({path:downloadURL})).then((data)=>{
